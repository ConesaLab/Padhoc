...
def install(destDir=None, downloadDir=None, redownload=False, compile=False, javaHome=None, updateLocalSettings=False):
    ...
    if compile:
        ...
        if javaHome is None or javaHome.strip() == "":
...
def makeConfigXML(workdir, bannerDir):
    ...
    ET.SubElement(eval, "modelFilename").text = bannerDir + "/output/cnobata_banner.bin"
    ...
    ET.SubElement(dictionaryTagger, "dictionaryFile").text = bannerDir + "/dict/HMDB-CHEBI.dict"
    ET.SubElement(dictionaryTagger, "dictionaryType").text = "METABOLITE"
    # Write to file
...
#        assert os.path.exists(bannerPath + libPath)
    ...
    cwd = os.getcwd()
    os.chdir(bannerPath)
    args = Settings.JAVA.split() + ["-cp", classPath, "-Xmx2g", "banner.eval.BANNER", "test", config]
    ...
    for line in mentionfile:
        ...
        if bannerId not in sentenceEntityCount:
            sentenceEntityCount[bannerId] = 50 #hack
        for ent in entities:
...
if __name__=="__main__":
    ...
    if not options.install:
        if os.path.isdir(options.input) or options.input.endswith(".tar.gz"):
            ...
            import TEES.Utils.STFormat.ConvertXML
            import TEES.Utils.STFormat.STTools
            options.input = TEES.Utils.STFormat.ConvertXML.toInteractionXML(TEES.Utils.STFormat.STTools.loadSet(options.input), options.inputCorpusName)

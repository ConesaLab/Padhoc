...
#	    print(sent.attrib["id"]+"|t|"+sent.attrib['text']+"\n"+sent.attrib["id"]+"|a|\n",file=sys.stderr)
    ...
    for line in f:
        ...
        if 't' in words[0]:
            ...
            if len(words)>1:
                ...
                #dictionnary with sentence id, position start, position end, metabolite name
    ...
    for sentence in root.findall("./document/"):#foreach sentence
        if sentence.get('id') in sentencemetabolite:
            sent=sentencemetabolite[sentence.get('id')]#result tmchem
            e=vale
		...
		print(tmpposi)
                ...
                #add node to the good sentence if the node is not already present
		vu=0;
		for entity in sentence:
			print(entity.attrib)
			#origBANNEROffset
			banneroffset=entity.attrib['origBANNEROffset']
			#print(banneroffset)
			tmpposientity=banneroffset.split('-')
			#print(tmpposientity[0])
			#print(tmpposientity[1])
			#print(tmpposientity)
			if float(tmpposi[1])<=float(tmpposientity[0]) :
#				print(tmpposi[1])
#				print(tmpposientity[0])
				if float(tmpposi[2])>float(tmpposientity[0]) :
					print(tmpposi[1])
	                                print(tmpposientity[0])
					print(tmpposi[2])
					print(tmpposientity[0])
					...
					print('first vu')
			if float(tmpposi[1])>float(tmpposientity[0]) :
				if float(tmpposi[1])<float(tmpposientity[1]) :
					print(tmpposi[1])
                                        print(tmpposientity[0])
                                        print(tmpposi[1])
                                        print(tmpposientity[1])
					...
					vu=1
		if vu==0:
	                newentity=xml.etree.ElementTree.SubElement(sentence,tag='entity',attrib={"charOffset":cos,"given":"True","id":ide,"origBANNEROffset":obo,"source":"TmChem","text":sent[posi],"type":"Metabolite"})#change metabolite by protein if used with the original TEES
        	        e=e+1
    #Write the output

...
#modifié par cecile pereira le 21/04/2012
my $usage =" Usage: inparanoid.pl <FASTAFILE with sequences of species A> <FASTAFILE with sequences of species B> <dossier avec les blasts> <dirOut>[FASTAFILE with sequences of species C]
...
$dossierblast=$ARGV[2];######ajout cecile
...
$fasta_seq_fileC = "$ARGV[4]" if ($use_outgroup); # This is outgroup file  

########modifications par cecile pour pouvoir laisser les blasts dans leur dossier de resultat
my $espA=$fasta_seq_fileA;
$espA=~s/\.fa$//;
my $espB=$fasta_seq_fileB;
$espB=~s/\.fa$//;
my $blast_outputAB = $dossierblast.$espA . "_" .$espB.'.blastINP';
my $blast_outputBA = $dossierblast.$espB . "_" .$espA.'.blastINP';
my $blast_outputAA = $dossierblast.$espA . "_" .$espA.'.blastINP';
my $blast_outputBB = $dossierblast.$espB . "_" .$espB.'.blastINP';
...
if ($output){
    open OUTPUT, ">/home/cecile/Bureau/miseajourBD/local/Inparanoid/inparanoid_cecile/result/output/$outputfile" or warn "Could not write to OUTPUT file $filename\n";
    system("pwd");
}
#################################################
...
Usage $0 <FASTAFILE with sequences of species A > $fasta_seq_fileA <FASTAFILE with sequences of species B> <FASTAFILE with sequences of species C>\n";
$id = 0;
while (<A>){
    if(/^\>/){
			++$id;
...
#		 next unless ($this_family); # There was an alternative BA match but it's best match did not belong here
    ...
    if ($mysql_table){
			$filename2 = $ARGV[3]."sqltable." . $ARGV[0] . "-" . $ARGV[1];
...
sub overlap_test{
	...
	$retval = 1;
#	if ($Fld[3] >= $Fld[4]) {
		print "lengths of the aggregate matching region on query ".$Fld[5]."\n"; 
		print "$seq_overlap_cutoff * $Fld[3])\n";
		print "$Fld[5] < ($seq_overlap_cutoff * $Fld[3])\n";

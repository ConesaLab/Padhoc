...
#
# NOTE: This script requires blastp (NCBI BLAST) version 2.2.26+ or higher, that supports 
# compositional score matrix adjustment (-C2 flag).

#19/04/2012
#cecile pereira
#modifications pour que le programme fonctionne avec la derniere version de blast


my $usage =" Usage: inparanoid.pl <FASTAFILE with sequences of species A> <FASTAFILE with sequences of species B> <dossier blast> <dossier de sortie> [FASTAFILE with sequences of species C]
...
$blastall = "/home/cecile/Bureau/ncbi-blast-2.2.26+/bin/blastp";#chemin du programme blastp modifié par cecile
$formatdb = "/home/cecile/Bureau/ncbi-blast-2.2.26+/bin/makeblastdb"; #chemin du programme makeblastdb modifié par cecile
$seqstat = "seqstat.jar";
...
$fasta_seq_fileC = "$ARGV[3]" if ($use_outgroup); # This is outgroup file  


$noextensionA = substr $fasta_seq_fileA, 0, -6;
$noextensionB = substr $fasta_seq_fileB, 0, -6;

my $blast_outputAB = $noextensionA . "-" . $noextensionB;
my $blast_outputBA = $noextensionB . "-" . $noextensionA;
my $blast_outputAA = $noextensionA . "-" . $noextensionA;
my $blast_outputBB = $noextensionB . "-" . $noextensionB;
...
if ($output){
    open OUTPUT, ">$ARGV[3]/$outputfile" or warn "Could not write to OUTPUT file $filename\n";
}
...
if ($run_blast){
    ...
    print STDERR  "$formatdb -dbtype prot -in $fasta_seq_fileA\n";
    system ("$formatdb -dbtype prot -in $fasta_seq_fileA");#ligne modifiee par cecile
    system ("$formatdb -dbtype prot -in $fasta_seq_fileB") if (@ARGV >= 2);#ligne modifiée par cecile
    system ("$formatdb -dbtype prot -in $fasta_seq_fileC") if ($use_outgroup);#ligne modifiée par cecile
...
#################################################
    $count = 0;
    open AB, "$ARGV[2]/$blast_outputAB" or die "Blast output file A->B $ARGV[2]/$blast_outputAB is missing\n";
    $old_idQ = 0;
    while (<AB>){
			chomp;
			...
			if( scalar @Fld < 9 ){
	   		 if($Fld[0]=~/done/){ #il n'y a jamais ecris "done" dans les fichiers de sortie blast parser...
					print STDERR "AB ok\n";
	    	 }
	   		 next;
			} 
...
#################################################
    $count = 0;
    open BA, "$ARGV[2]/$blast_outputBA" or die "Blast output file B->A is missing\n";
    $old_idQ = 0;
    while (<BA>){
	chomp;
	@Fld = split(/\s+/);    # Get query, match and score
...
################################
    $count = 0;
    $max_hit = 0;
    open AA, "$ARGV[2]/$blast_outputAA" or die "Blast output file A->A is missing\n";
    while (<AA>) {
	chomp;                  # strip newline
...
################################
    $count = 0;
    open BB, "$ARGV[2]/$blast_outputBB" or die "Blast output file B->B is missing\n";
    while (<BB>) {
	chomp;                  # strip newline
...
###################
    ...
    if ($html){
	open HTML, ">$ARGV[3]/$htmlfile" or warn "Could not write to HTML file $filename\n";
    }
...
# ##############################################################################
	...
	open FF, ">$ARGV[3]/BS_vs_bits" or warn "Could not write to file BS_vs_bits\n";
    }
    for $i(1..$o){
	@membersA = split(/ /, $paralogsA[$i]);
	@membersB = split(/ /, $paralogsB[$i]);		 
...
#		 next unless ($this_family); # There was an alternative BA match but it's best match did not belong here
	...
	open F2, ">$ARGV[3]/$filename2" or die;
	for $i(1..$o){
	    @membersA = split(/ /, $paralogsA[$i]);
	    for $m (@membersA){
		# $m =~ s/://g;
...
sub bootstrap{
    ...
    system "$formatdb -dbtype prot -in $db_file";#modif cecile
    # Use soft masking in 1-pass mode for simplicity.
    #system "$blastall -F\"m S\" -i $query_file -z 5000000 -d $db_file -p blastp -M $matrix -m7 | ./$blastParser 0 -a > $seq_id2.faa";#modif cecile
    system("$blastall -query $query_file -db $db_file -matrix $matrix -dbsize 5000000 -outfmt 5 | ./$blastParser 0 -a > $seq_id2.faa");
...
sub do_blast_1pass {
  ...
  #system ("$blastall -F\"m S\" -i $Fld[0] -d $Fld[1] -p blastp -v $Fld[3] -b $Fld[3] -M $matrix -z 5000000 -m7 | ./$blastParser $score_cutoff > $Fld[4]");#modif cecile
  print  STDERR "$blastall -query $Fld[0] -db $Fld[1] -matrix $matrix -dbsize 5000000 -seg yes -outfmt 5 | ./$blastParser $score_cutoff > $Fld[4]\n";
  system("$blastall -query $Fld[0] -db $Fld[1] -matrix $matrix -dbsize 5000000 -seg yes -outfmt 5 | ./$blastParser $score_cutoff > $Fld[4]");#modif cecile
...
sub do_blast_2pass {
	...
	system("date");
	#open FHR, "$blastall -C3 -F\"m S\" -i $Fld[0] -d $Fld[1] -p blastp -v $Fld[3] -b $Fld[3] -M $matrix -z 5000000 -m7 | ./$blastParser $score_cutoff|"; modif cecile
	open FHR, "$blastall -comp_based_stats 3 -query $Fld[0] -db $Fld[1] -max_target_seqs $Fld[3] -matrix $matrix -dbsize 5000000 -outfmt 5 | ";#modif cecile
	...
	foreach $aQuery (keys % theHits) {
		...
		system ("$formatdb -dbtype prot -in $tmpd");#modif cecile
		#system ("$blastall -C0 -FF -i $tmpi -d $tmpd -p blastp -v $Fld[3] -b $Fld[3] -M $matrix -z 5000000 -m7 | ./$blastParser $score_cutoff >> $Fld[4]");#modif cecile
		system("$blastall -comp_based_stats 0 -query $Fld[0] -db $Fld[1]  -max_target_seqs $Fld[3] -matrix $matrix -dbsize 5000000 -outfmt 5 | ./$blastParser $score_cutoff >> $Fld[4]");
	}
